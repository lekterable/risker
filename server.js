...
websocket.on('connection', (socket) => {
  ...
  socket.broadcast.emit('info', {players})
    ...
    if(websocket.sockets.connected[data])
        ...
        if(res){
          games = [...games, {host: socket.id, turn: socket.id, players: [{id: socket.id, total: 0, round: 0, roll: [0,0]}, {id: data, total: 0, round: 0, roll: [0,0]}]}]
  ...
  socket.on('ready', (callback) => {
    callback(games.find((game) => game.players.some((player) => player.id === socket.id)))
  })
  socket.on('turn-roll', () => {
    ...
    if(dices.some((dice) => dice === 1)){
      games = games.filter((game) => game !== currentGame)
      currentGame = {...currentGame, turn: opponent.id, players:[{...opponent, roll: [0,0]}, {...player, round: 0, roll: dices}]}
      games = [...games, currentGame]
    } else {
      currentGame = {...currentGame, players:[{...opponent}, {...player, round: player.round + dices.reduce((prev, curr) => prev + curr, 0), roll: dices}]}
      games = [...games, currentGame]
    }
  ...
  socket.on('disconnect', ()=>{
    games = games.filter((game) => !game.players.some((player) => player.id === socket.id))
